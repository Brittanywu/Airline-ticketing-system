...
{	
	...
	while (fscanf(Flight_list, "%s%s%s%s%s%s%d%d%d%d%s%s%s%s", Flight.Flightnum, Flight.Flighttime_up,
		...
		Flight.Company,Flight.Flighttime_down_date) != EOF)
	{
...
{	
	...
	{	
		...
		if (strcmp(Now.Downland, Past.Downland) == 0 && strcmp(Now.Startland, Past.Startland) == 0)
		{
...
int in_Put(FlightInfo* Flight_new)
{
	...
	while (sum > 0)
	{
		...
		if (num >= '0'&&num <= '8'&&last - sum == 1)
		{
			...
			/*int len = strlen(in[Has_in_put]);
			for (int i = 0; i < len; i++) 
			{
				in[Has_in_put][i] = '\0';
			}
			for (int i = Has_in_put; i < sum; i++)
			{
				len = strlen(in[i+1]);
				for (int j = 0; j < len; j++)
				{
	...
	while (1)
	{
		...
		switch (Command)
		{
			case '1':
			{
				...
				Flight_new->Ontimerate[3] = '3';
				Flight_new->Ontimerate[4] = '9';
...
void add_Flight()
{
	...
	if (Is_ok) {
		while (1)
		{
			...
			while (fscanf(fp, "%s%s%s%s%s%s%d%d%d%d%s%s%s", Flight.Flightnum, Flight.Flighttime_up,
				Flight.Flighttime_down, Flight.Startland, Flight.Downland, Flight.Flighttype, &Flight.Firstfare,
				&Flight.Economfare, &Flight.Firstrest, &Flight.Economrest, Flight.Pilot_1, Flight.Pilot_2,
				Flight.Company) != EOF)
			{
